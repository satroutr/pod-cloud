...
class IPMItool(object):
    ...
    def connect(self, cimcip, username, password):
        """
        :param cimcip: The console dns or ip address
        ...
        self.args = ['-I', 'lanplus', '-H', self.cimcip, '-U', self.username, '-P',  self.password]
...
#             self.method = self._expect_method
    ...
    def execute(self, command):
        ...
        if isinstance(command, str):
            self._subprocess_method(command.split())
        elif isinstance(command, list):
            self._subprocess_method(command)
        else:
